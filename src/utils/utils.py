...
#New Shizzz
def trim_video(input_path, max_frames=900):
    # Open the video file
    ...
    num_frames_to_keep = min(total_frames, max_frames)

    # Create VideoWriter object to save the trimmed video
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    output_path = input_path  # Overwrite the input video
    out = cv2.VideoWriter(output_path, fourcc, fps, (int(cap.get(3)), int(cap.get(4))))

    # Read and write frames up to the specified limit
    for _ in range(num_frames_to_keep):
        ret, frame = cap.read()
        if not ret:
            break
        out.write(frame)
...
def extract_predict_annotate(output_dir, ensemble_models, video_glob, video_idxs, transformer, blazeface_dir, device, model_list):
    ...
    for ne in tqdm(video_idxs, desc='Annotating videos'):
        ...
        while success:
            ...
            h, w, c = img.shape
            # Debugging Code
            # print(f"Image dimensions: (h, w, c) = ({h}, {w}, {c})")
